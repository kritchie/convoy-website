<template>
	<div>
		...
		<div class="max-w-[1200px] m-auto -mt-130px mb-88px">
			...
			<div class="bg-[#ececf4] rounded-8px w-fit m-auto flex flex-row mb-54px">
				<li class="list-none" v-for="tab of tabs" :key="tab.id">
					<button class="rounded-6px py-12px px-8px min-w-[86px] desktop:min-w-[132px]" :class="activeTab === tab.id ? 'active' : ''" @click="switchTabs(tab.id)">
						<span class="text-grey-40 text-12 tracking-[0.02em] desktop:text-14">{{ tab.label }}</span>
					</button>
				</li>
			</div>

			<section class="download bg-[#f3f3f8] rounded-4px p-24px max-w-[724px] w-full m-auto">
				...
				<div v-if="activeTab == 'linux'">
					<ul class="flex flex-row m-auto w-full mb-24px border-b border-b-grey-10">
						<li v-for="tab of linuxTabs" :key="tab.id" class="mr-24px !list-none last-of-type:mr-0">
							<button class="pb-10px flex items-center has-icon" :class="linuxActiveTab === tab.id ? 'activeTab' : ''" @click="switchLinuxTabs(tab.id)">
...
.active {
	@apply bg-white-100 shadow-sm transition-all duration-300;
	span {
