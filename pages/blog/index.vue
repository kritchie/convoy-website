<template>
	<div class="m-auto pb-0 flex justify-between pt-0">
		<aside class="w-240px hidden desktop:sticky desktop:top-150px desktop:block desktop:pl-20px desktop:pr-32px">
			<ul class="p-0">
				<h3 class="font-bold text-14 text-black mb-18px">CATEGORIES</h3>

				<li v-for="(tag, index) in tags" :key="'tag' + index" class="font-semibold text-14 mb-30px text-grey-60">
					<nuxt-link :to="tag !== 'All Posts' ? '/blog?tag=' + tag : '/blog'">{{ tag }}</nuxt-link>
				</li>
			</ul>

			<!-- Pending when there is enough content for this -->
			<!-- <form class="bg-white-100 rounded-4px w-full flex border border-grey-20">
		...
		<main class="max-w-[1035px] w-full px-20px">
			<div class="relative">
				<h1 class="font-bold text-black flex items-center">
			...
			<div class="rounded-8px shadow-card max-w-[970px] bg-white-100 mt-32px pt-32px px-12px desktop:pl-56px desktop:pt-56px desktop:pr-0 desktop:flex desktop:justify-between desktop:flex-wrap desktop:items-end mobile:mb-48px" v-if="featurePosts.length > 0">
			...
			<div class="newsletter card flex flex-col items-center max-w-[970px] py-32px px-24px desktop:px-70px mt-40px desktop:mt-48px desktop:flex-row desktop:justify-around">
				<div>
					<p class="mt-10px text-14 text-center desktop:text-left">Join our newsletter</p>
			...
			<div class="desktop:grid desktop:grid-cols-2 gap-y-62px gap-x-48px max-w-[970px] mb-48px">
